...
在 Windows 測試人員組建19041.1346 中，我們會在 HoloLens 2 設定應用程式中新增憑證檢視器。 證書安裝目前支援 .cer 與 .crt 檔案。 裝置擁有者可以在本機電腦和目前使用者中安裝憑證; 所有其他使用者只能安裝至目前的使用者。 使用者只能從 [設定] UI 中移除直接安裝的憑證。 如果憑證已透過其他方式安裝，則也必須使用相同的機制來移除。
...
#### 若要使用證書檢視器安裝證書： 
1. 流覽至 [**設定] App**的 [  ->  **更新與安全性**  ->  **憑證**]，然後選取 [**安裝憑證**]。 
1. 從檔案選擇器體驗中選取 .cer 檔案。
1. 選取 [本機電腦] (或您擁有憑證的位置。 ) 
1. 選取 [ **根目錄** ] 做為證書存放區 (或您要放置憑證的商店。 )  
1. 按一下 **\[安裝\]**。

現在應該在裝置上安裝證書。

#### 若要使用憑證檢視器移除憑證： 
1. 流覽至 [**設定] App**的 [  ->  **更新與安全性**  ->  **憑證**]。
1. [搜尋] 方塊中的 [依名稱搜尋憑證]。
1. 選取憑證。
1. 按一下 [**移除**]
