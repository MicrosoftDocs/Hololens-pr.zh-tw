...
- 將憑證套用到裝置
- 啟用開發人員模式
- 設定 Kiosk 模式（您可以在[這裡](hololens-kiosk.md#use-a-provisioning-package-to-set-up-a-single-app-or-multi-app-kiosk)找到配置 kiosk 模式的詳細指示。
...
- 設定 HoloLens 首次體驗（OOBE）
- 設定 Wi-fi 網路
- 在 Azure Active Directory 中註冊裝置，或建立本機帳戶
- 新增憑證
- 啟用開發人員模式
- 設定 kiosk 模式。 （您可以[在這裡](hololens-kiosk.md#use-a-provisioning-package-to-set-up-a-single-app-or-multi-app-kiosk)找到配置 kiosk 模式的詳細指示）。
...
2. **選項2：** [從適用于 Windows 10 的 windows 評量與部署套件（ADK）](https://developer.microsoft.com/windows/hardware/windows-assessment-deployment-kit)。 如果您從 Windows ADK 安裝 Windows 配置設計工具，請從 [**選取您要安裝的功能**] 對話方塊中選取 [**配置設計**工具]。 此選項不含 HoloLens 2 功能。
...
> [!NOTE]
> 您在 [**高級] 配置**中建立的置備套件，不需要在 Windows 全息版中加入版本升級授權，就能成功套用至 HoloLens （第1代）。 [詳細資訊請參閱適用于 HoloLens 的 Windows 全息版（第1代）](hololens1-upgrade-enterprise.md)。
...
6. 使用[本文稍後所述](#what-you-can-configure)的任何設定，展開 [執行時間]**設定**並自訂套件。
    ...
    > [!IMPORTANT]
    > （適用于 Windows 10，僅限版本1607）如果您在預配套件中建立本機帳戶，您必須使用每42天的 [**設定**] app 來變更密碼。 如果在該期內沒有變更密碼，帳戶會被鎖，因而無法登入。 如果帳戶已鎖定，您必須[執行完整的裝置修復](https://developer.microsoft.com/windows/mixed-reality/reset_or_recover_your_hololens#perform_a_full_device_recovery)。
...
1. 使用 USB 纜線將裝置連接至電腦，然後啟動裝置。 請勿繼續超過 OOBE 的**第一個 interactable 時刻**頁面。   
    - 在 HoloLens （第1代）中，此頁面包含藍色方塊。 
...
5. 在 **\[調整\]** 頁面上時，再次同時快速按下再放開 **\[音量降低\]** 和 **\[電源\]** 按鈕。
...
> [!NOTE]
> HoloLens 目前不支援使用預配套件來安裝應用程式（**UniversalAppInstall**）。
