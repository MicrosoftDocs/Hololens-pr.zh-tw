...
您可以將 HoloLens 裝置設定為固定用途的裝置（亦稱為*kiosk*），方法是將裝置設定為在 kiosk 模式中執行。 Kiosk 模式會限制裝置上可用的應用程式（或使用者）。 Kiosk 模式是一種便利的功能，您可以用來將 HoloLens 裝置專用於商務應用程式，或在應用程式示範中使用 HoloLens 裝置。
...
> [!IMPORTANT]  
> Kiosk 模式決定使用者登入裝置時可使用哪些 app。 不過，kiosk 模式不是安全性方法。 它不會停止「允許」 app，無法開啟其他不允許的應用程式。 為了封鎖 app 或進程的開啟，請使用[Windows Defender 應用程式控制（WDAC） CSP](https://docs.microsoft.com/windows/client-management/mdm/applicationcontrol-csp)來建立適當的原則。  
...
若要設定 HoloLens （第1代）裝置使用 kiosk 模式，您必須先確定裝置執行的是 Windows 10 版本1803或更新版本。 如果您已使用 Windows 裝置恢復工具將 HoloLens （第1代）裝置復原至預設組建，或者如果您已安裝最新的更新，則您的裝置已準備好進行設定。
...
| --- | --- | --- | --- |
|單一應用程式資訊站 |Azure Active Directory （AAD）或本機帳戶中的 Managed Services 帳戶（MSA） |是 |否 |
|多應用程式亭 |AAD 帳戶 |否 |是 |
...
|只執行自訂應用程式的裝置。 |此裝置充當大多數使用者的展臺（僅執行自訂應用程式），但作為特定使用者群組的標準裝置使用。 |

### 規劃展臺應用程式

如需如何選擇 kiosk app 的一般資訊，請參閱[選擇適用于指派存取權的 app （kiosk 模式）的指導方針](https://docs.microsoft.com/windows/configuration/guidelines-for-assigned-access-app)。

如果您使用 Windows Device Portal 來設定單一應用程式亭，您可以在安裝程式期間選取 app。  

如果您使用行動裝置管理（MDM）系統或配包來設定 kiosk 模式，您可以使用[AssignedAccess Configuration Services 提供者（CSP）](https://docs.microsoft.com/windows/client-management/mdm/assignedaccess-csp)來指定應用程式。 CSP 會使用[應用程式使用者模型識別碼（aumid）](https://docs.microsoft.com/windows/configuration/find-the-application-user-model-id-of-an-installed-app)來識別應用程式。 下表列出一些您可以在多應用程式亭中使用的 Aumid 應用程式。
...
Kiosk 配置設定檔包括 [**使用者登入類型**] 設定。 **使用者登入類型**會識別可以使用您所新增之應用程式的使用者（或包含使用者的群組）。 如果使用者使用未包含在設定檔中的帳戶登入，該使用者就無法在展臺上使用 app。  
...
您可以針對裝置和使用者使用單一群組（群組1）。 一個裝置和使用者 A、B 和 C 都是這個群組的成員。 您可以設定展臺設定檔，如下所示：  

- **使用者登入類型**：群組1
- **已指派的群組**：群組1

不論哪個使用者先登入裝置（並移至全新體驗或 OOBE），資訊站設定都會部署到裝置。 使用者 A、B 和 C 都可以登入裝置並取得展臺體驗。
...
### <a id="mdmconfigsingle"></a>MDM，步驟3（單一應用程式） &ndash; 設定單一應用程式資訊站的設定
...
1. 選取 [**使用者登入類型**  >  （**本機使用者帳戶**）]，然後輸入可登入資訊站的本地（裝置）帳戶或 Microsoft 帳戶（MSA）的使用者名稱。
   > [!NOTE]  
...
### <a id="mdmconfigmulti"></a>MDM，步驟3（多重應用程式） &ndash; 設定多應用程式資訊站的設定
...
- 您也可以選擇在 Intune 或其他 MDM 服務中使用自訂開始配置。 如需詳細資訊，請參閱在[MDM （Intune 及其他人）中啟動版面設定檔](#start-layout-file-for-mdm-intune-and-others)。
...
### <a id="mdmsingledeploy"></a>MDM，步驟5（單應用程式） &ndash; 部署單一應用程式亭
...
### <a id="mdmmultideploy"></a>MDM，步驟5（多重應用程式） &ndash; 部署多應用程式亭
...
- 請勿包含傳統型 Windows 應用程式（Win32）。 HoloLens 不支援這些應用程式。
...
> [!NOTE]  
> 如果您使用[MDM](#use-microsoft-intune-or-other-mdm-to-set-up-a-single-app-or-multi-app-kiosk)來設定多個 app 的展臺，您可以選擇使用 [開始] 版面配置。 如需詳細資訊，請參閱[MDM （Intune 及其他）的預留位置啟動版面](#start-layout-file-for-mdm-intune-and-others)配置檔案。
...
#### <a id="start-layout-file-for-mdm-intune-and-others"></a>MDM 的 Placeholder 開始版面配置檔案（Intune 及其他）

將下列範例儲存為 XML 檔案。 當您在 Microsoft Intune （或另一個提供展臺設定檔的 MDM 服務）中設定多應用程式亭時，您可以使用這個檔案。
...
1. **選用**。 （如果您想要在裝置的初始設定之後套用預配套件，且展臺裝置上已提供系統管理員使用者，請略過這個步驟。）選取 [**執行時間設定** &gt; **帳戶** &gt; **使用者**]，然後建立使用者帳戶。 提供使用者名稱和密碼，然後選取 [ **UserGroup**系統  >  **管理員**]。  
  
     您可以使用此帳戶來查看 [預配狀態與記錄]。  
1. **選用**。 （如果您在展臺裝置上已經有非系統管理員帳戶，請略過這個步驟。）選取 [**執行時間設定** &gt; **帳戶** &gt; **使用者**]，然後建立本機使用者帳戶。 請確定使用者名稱與您在配置 XML 中指定的帳號相同。 選取 [ **UserGroup**  >  **標準使用者**]。
1. 選取 **[**  >  **儲存**檔案]。
...
- 您也可以[在安裝完成後將預配套件套用至 HoloLens](hololens-provisioning.md#4-apply-a-provisioning-package-to-hololens-after-setup)。
